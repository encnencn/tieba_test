...
def _fetch_like_tieba_list():
    print
    u'获取喜欢的贴吧ing...' if system_env else '获取喜欢的贴吧ing...'
    page_count = 1
    find_like_tieba = []
    while True:
...
def _make_sign_request(tieba, fid, tbs, BDUSS):
    ...
    sign_request = {"BDUSS": BDUSS,
                    ...
                    "fid": fid, "kw": tieba, "net_type": "3", 'tbs': tbs}
...
def _handle_response(sign_resp):
    ...
    if error_code == '0':
        print
        u"签到成功,经验+%d" % sign_bonus_point if system_env else "签到成功,经验+%d" % sign_bonus_point
    else:
        error_msg = sign_resp['error_msg']
        if error_msg == u'亲，你之前已经签过了':
            print
            u'之前已签到' if system_env else '之前已签到'
        else:
            print
            u'签到失败' if system_env else '签到失败'
            print
            "Error:" + unicode(error_code) + " " + unicode(error_msg)
...
def _sign_tieba(tieba, BDUSS):
    ...
    if not already_sign:
        print
        tieba.decode('utf-8') + u'......正在尝试签到' if system_env else tieba + '......正在尝试签到'
    else:
        if already_sign[0] == "已签到":
            print
            tieba.decode('utf-8') + u"......之前已签到" if system_env else tieba + "......之前已签到"
            return

    if not fid or not tbs:
        print
        u"签到失败，原因未知" if system_env else "签到失败，原因未知"
        return
...
def sign(my_cookie, BDUSS):
    ...
    if len(_like_tieba_list) == 0:
        print
        u"获取喜欢的贴吧失败，请检查Cookie和BDUSS是否正确" if system_env else "获取喜欢的贴吧失败，请检查Cookie和BDUSS是否正确"
        return
    thread_list = []
